...
google = "https://maps.google.com/maps/api/geocode/json?address="
googleAPI = "&keys=AIzaSyBeWepFeeW1In8VxNPbh7SiBou-pkuVR_M"

for key, value in states_caps.items():
    #print ("$('#" + key + "').css('fill', 'red')")
    temp = value['capital']
    temp = temp.replace(' ','+')
    address = google + temp + googleAPI

    googleDic = urllib.request.urlopen(address)
    loc = json.loads(googleDic.read().decode(googleDic.info().get_param('charset') or 'utf-8'))
    lat = loc['results'][0]['geometry']['location']['lat']
    lng = loc['results'][0]['geometry']['location']['lng']
    lat = str(lat)
    lng = str(lng)

    string1 = "https://api.forecast.io/forecast/e92382f8ae8242410ad7dc378acf19ec/"
    a = string1 + lat + "," + lng
    b = urllib.request.urlopen(a)
    c = json.loads(b.read().decode(b.info().get_param('charset') or "utf-8"))
    temp = c['currently']['temperature']
    print (temp)

    if temp <= 10:
        print ("$('#" + key + "').css('fill', 'blue')")
    elif 10 < temp <= 30:
        print ("$('#" + key + "').css('fill', 'cyan')")
    elif 30 < temp <= 50:
        print ("$('#" + key + "').css('fill', 'green')")
    elif 50 < temp <= 80:
        print ("$('#" + key + "').css('fill', 'orange')")
    elif temp > 80:
        print ("$('#" + key + "').css('fill', 'red')")
print ('''
});
