...
def Unigrams(corpus, training):
    ...
    # common words to not include in the model
    common = ["the", "a", "be", "to", "of", "an", "in", "with", "at", "and", "have", "that", "I", "for"]
    ...
    total = 0;
    for key in model:
        model[key] = model[key] / (numWords + (0.1 * len(model.keys())))
        total += model[key]
...
def TestModel(posModel, negModel, probPosClass, probNegClass, test):
    ...
    f = open("TenHoeve-Adam-extra-out.txt", "w")
    for i in range(len(result)):
        f.write(ids[i] + result[i] + "\n")
        print(ids[i] + result[i])
    f.close()

    print("Percent T:", posses / (posses + negs))
    print("Percent F:", negs / (posses + negs))
...
def main():
    ...
    posText = open("hotelT-train.txt", encoding = "utf8").read()
    posText = posText.split("\n")
    posText = np.array(posText)
    negText = open("hotelF-train.txt", encoding = "utf8").read()
...
#     negTrain, negTest = train_test_split(negText, test_size = 0.2, random_state = 99)
    ...
    posTrain = posText
    negTrain = negText
#     test = open("data.txt", encoding = "utf8").read()
    test = open("data.txt").read()
...
#     resultpos = TestModel(posModel, negModel, probPosClass, probNegClass, posTest)
#     resultneg = TestModel(posModel, negModel, probPosClass, probNegClass, negTest)
    result = TestModel(posModel, negModel, probPosClass, probNegClass, test);
