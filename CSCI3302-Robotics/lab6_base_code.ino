...
bool goal_changed = FALSE; // Track if the goal coordinates have been changed since we last path planned.
...
short *reconstruct_path(short *prev, int source_vertex, int dest_vertex) {
  ...
  if (path[last_idx - 1] != source_vertex) {
    final_path = new short[1];
    final_path[0] = -1;
    return final_path;
  }
...
void loop () {
  ...
  /****************************************/
  if(curr_state == STATE_START){
    int i, j;
    xy_coordinates_to_ij_coordinates(pose_x, pose_y, &i, &j);
    prev = run_dijkstra(world_map, ij_coordinates_to_vertex_index(i, j));

    //BH: Need to free your memory once you're done using it, i.e.:
    if (path != NULL) { delete path; path = NULL; }
    path = reconstruct_path(prev, ij_coordinates_to_vertex_index(i, j), ij_coordinates_to_vertex_index(goal_i, goal_j));
    idx = 0;
    sparki.clearLCD();
    ...
    delay(3000);
    if(path[0] != -1){ 
      curr_state = STATE_HAS_PATH;
    }
  } else if(curr_state == STATE_HAS_PATH){
      if(!goal_changed){ 
      //FINDS THE CURRENT VERTEX
        vertex_index_to_ij_coordinates(path[idx], &i, &j); 
        ij_coordinates_to_xy_coordinates(i, j, &x, &y); 
        
        //FIND THE VERTEX TO DETERMINE THETA
        
        if(path[idx] != -1){
          if(idx+1 < 16 && path[idx + 1] != -1){
            vertex_index_to_ij_coordinates(path[idx + 1], &i1, &j1); 
            ij_coordinates_to_xy_coordinates(i1, j1, &x1, &y1);
//            if(idx+2 < 16 && path[idx + 2] != -1){
//              vertex_index_to_ij_coordinates(path[idx + 2], &i2, &j2); 
//              ij_coordinates_to_xy_coordinates(i2, j2, &x2, &y2);
//              t = atan2(y2-y1, x2-x1); 
//            }
            t = atan2(y1-y, x1-x);
          } 
          set_pose_destination(x, y, t); 
          idx++;
        }
        curr_state = STATE_SEEKING_POSE;
      } else {
        curr_state = STATE_START;
        goal_changed = FALSE;
      }
  } else if(curr_state == STATE_SEEKING_POSE){
    // You should probably just set this once, up in the state above, rather than every cycle in STATE_SEEKING_POSE
