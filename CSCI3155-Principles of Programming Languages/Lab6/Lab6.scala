...
object Lab6 extends jsy.util.JsyApplication with Lab6Like {
   ...
   *
   *   star ::= atom stars
   *   stars ::= epsilon | '*' stars | '+' stars | '?' stars
   *
   *   atom ::= c | . | ! | # | (re)
   * 
   */
  object REParser extends REParserLike {
      ...
      case Success(r, next) => {
        def concats(acc: RegExpr, next: Input): ParseResult[RegExpr] = {
          if (next.atEnd) Success(acc, next)
            ...
            case Success(ret, nex) => concats(RConcat(acc, ret), nex)
            case _ => Success(acc, next)
          }
        }
        concats(r, next)
      ...
      case Success(r, next) => {
        def stars(acc: RegExpr, next: Input): ParseResult[RegExpr] = {
          if(next.atEnd) Success(acc, next)
            ...
            case ('*', nex) => stars(RStar(acc), nex)
            case ('+', nex) => stars(RPlus(acc), nex)
            case ('?', nex) => stars(ROption(acc), nex)
            case _ => Success(acc, next)
          }
        }
        stars(r, next)
    ...
    def atom(next: Input): ParseResult[RegExpr] = next.first match {
      ...
      case '(' => re(next.rest) match {
        case Success(r, next) => next.first match {
          case ')' => Success(r, next.rest)
          case _ => Failure("expected )", next)
        }
        case failure => failure
      }
      case c if delimiters.contains(c) => Failure("not atom", next)
      case c => Success(RSingle(c), next.rest)
    }
  }
  ...
  def test(re: RegExpr, chars: List[Char])(sc: List[Char] => Boolean): Boolean = (re, chars) match {
    ...
    case (RConcat(re1, re2), _) => test(re1, chars){charsp => if (charsp.length > chars.length) false else test(re2, charsp)(sc)}
    case (RUnion(re1, re2), _) => test(re1, chars)(sc) || test(re2, chars)(sc)
    case (RStar(re1), _) => {
      if (chars.length == 0) sc(Nil)
      else sc(chars) || test(re1, chars) {charsp =>
        if (chars.length <= charsp.length) false
        else test(RStar(re1), charsp)(sc)
      }
    }
    ...
    case (RPlus(re1), _) => test(RUnion(re1, RStar(re1)), chars)(sc)
